...
package com.dave.events {
	...
	public class EventCenter {
		...
		public static function subscribe(type:String, listener:Function) {
			if (disp == null) { disp = new EventDispatcher(); }
			disp.addEventListener(type, listener);
		}
		
		public static function unsubscribe(type:String, listener:Function) {
			if (disp == null) { disp = new EventDispatcher(); }
			disp.removeEventListener(type, listener);
		}
		
		public static function broadcast(eventName:String, paramObj:Object) {
			if (disp == null) { disp = new EventDispatcher(); }
